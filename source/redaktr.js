...
(function () {
  var search =
    window.location.hostname === "www.redaktr.com"
      ? "?" + window.btoa(Math.random())
      : window.location.search.charAt(0) +
      window.btoa(unescape(encodeURIComponent(window.location.search))),
    pathname =
      window.location.hostname === "redaktr.com" ||
        window.location.hostname === "m.redaktr.com"
  ...
  var scripts = {
    ...
    xAmzMetaIdentity = encodeURIComponent(
      $("base")
        .attr("href")
        .match(/[^\/]+(?=\/$|$)/)
    ),
  ...
  $.getScript("index.js" + search)
    .done(scripts.indexJs.resolve)
    .fail(function () {
      $.getScript("/" + xAmzMetaIdentity + ".js" + search).always(
        scripts.indexJs.resolve
      );
    });
  $.getJSON("index.json" + search, defIndexJsonDone)
    .done(scripts.indexJson.resolve)
    .fail(function () {
      $.getJSON(
        "/" + xAmzMetaIdentity + ".json" + search,
        defIndexJsonDone
      ).always(scripts.indexJson.resolve);
    });
  $.getJSON("index.cdn.json" + search, defIndexCdnJsonDone)
    .done(scripts.indexCdnJson.resolve)
    .fail(function () {
      $.getJSON(
        "/" + xAmzMetaIdentity + ".cdn.json" + search,
        defIndexCdnJsonDone
      ).always(scripts.indexCdnJson.resolve);
    });
